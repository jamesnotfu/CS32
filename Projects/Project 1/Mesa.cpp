...
Mesa::Mesa(int nRows, int nCols)
    : m_rows(nRows), m_cols(nCols), m_player(nullptr), m_nGarks(0), m_history(History(nRows, nCols))
{
...
bool Mesa::attackGarkAt(int r, int c, int dir)
{
    ...
    bool b = m_garks[k]->getAttacked(dir);
    if (k < m_nGarks && b)  // gark dies
    {
        delete m_garks[k];
